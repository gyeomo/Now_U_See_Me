...
def train_net(args):
    ...
    else: #checkpoint =TRUE 이후로 계속 진행 
    ...
    if args.focal_loss:
        ...
        criterion = nn.CrossEntropyLoss().to(device) #defalut

    # Custom dataloaders
    train_dataset = ArcFaceDataset('train')
    train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True)
    #지정된 epoch수만큼 지났을 때 학습률 조정 
...
def train(train_loader, model, metric_fc, criterion, optimizer, epoch, logger):
    ...
    for i, (img, label) in enumerate(train_loader):
        ...
        # Forward prop.
        feature = model(img)  # embedding => [N, 512] 512특징으로 뽑고
        output = metric_fc(feature, label)  # class_id_out => [N,num_classes]분류 
